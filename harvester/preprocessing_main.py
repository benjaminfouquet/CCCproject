...
def format_timestamp(json_dict):
    ...
    time = time.astimezone(timezone('Australia/Melbourne'))
...
def test(filename):
    json_dict = read_sub(filename)
    json_dict['text'] = 'food, pasta, teriyaki, curry'
    print(update(json_dict))

    # read tweets within a list
    path = "reconstructed_process/reconstructed_process/real_data"
...
#test("reconstructed/reconstructed/tweetsStream.json")